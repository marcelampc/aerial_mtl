...
class DenseUNet(nn.Module):
    ...
    def __init__(self, d_block, input_nc=3, outputs_nc=[1], growth_rate=32,
                 ...
                 bilinear_trick=False, outputSize=[427, 571], tasks=['depth'], mtl_method='eweights'):
        ...
        # for GradNorm
        if 'gradnorm' in self.mtl_method:
        ...
        elif 'alexnorm' in self.mtl_method:
            self.tm = TestModule()
    ...
    def get_last_common_representation(self):
        return self.last_common_layer_fm

    def decoder_forward(self, out_d3, i_task):

        # input is (ngf * 2) x 64 x 64
        # out_d2 = self.decoder_tasks.taski_block2()
        out_reg_d2 =self.decoder_tasks[i_task].d_block2(self.get_decoder_input(self.intermediate_feature_maps[0], out_d3))
        # input is ngf x 128 x 128
        out_reg_d1 =self.decoder_tasks[i_task].d_block1(self.get_decoder_input(self.intermediate_feature_maps[1], out_reg_d2))
        # input is ngf x 256 x 256
        out_reg = self.decoder_tasks[i_task].last_conv(out_reg_d1)
    ...
    def forward(self, x):
        ...
        self.last_common_layer = self.decoder.d_block3
        self.last_common_layer_fm = out_d3 # feature map
        output = []
        
        if 'alexnorm' in self.mtl_method:
            out_d3_tuple = self.tm(out_d3)
        if self.mtl_method == 'mgda_approx':
            self.intermediate_feature_maps = [tb_denseblock1, out_conv1]

        # if 'depth' in self.tasks:
        for task_i in range(len(self.tasks)):
            if isinstance(out_d3, tuple):
                out_d3 = out_d3_tuple[task_i]
            # input is (ngf * 2) x 64 x 64
            # out_d2 = self.decoder_tasks.taski_block2()
...
class TestFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output1, grad_output2):
        s1 = (torch.sqrt((grad_output1**2).sum()) + 1e-6)
        s2 = (torch.sqrt((grad_output2**2).sum()) + 1e-6)
        grad_output1 = grad_output1 / s1
        grad_output2 = grad_output2 / s2
        return (grad_output1 + grad_output2) * (s1+s2)/2
...
class TestModule(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        return TestFunction.apply(input)