...
class MultiTaskGen(TrainModel):
    ...
    def initialize_semantics(self):
        ...
        t_index = self.opt.tasks.index('semantics')
        self.opt.n_classes = self.opt.outputs_nc[t_index]
        ...
        if self.total_iter % self.opt.print_freq == 0:
            ...
            with torch.no_grad():
                ...
                for k in range(output.shape[0]):
                    self.rmse += sqrt(self.mse_scaled_error(output[k], target[k], no_mask=self.opt.no_mask, valid_pxs=n_valid_pixls).item()) # mean through the batch
                    self.n_images += 1

            self.set_current_visuals(depth_gt=target.data[0],
                                        depth_out=output.data[0])
    ...
    def _train_batch(self):
        ...
        for i_task, task in enumerate(self.opt.tasks):
            ...
            if task == 'semantics':
                ...
                losses.append(self.criterion_instance(target, outG[i_task]))
                self.get_errors_instance(target, outG[i_task])
            elif task == 'normals':
                pass 
                losses.append(self.criterion_normals(target, outG[i_task]))
                self.get_errors_normals(target, outG[i_task])
